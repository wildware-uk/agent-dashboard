...
export const listTasksTool: McpTool<typeof inputSchema> = {
	...
	config: {
		...
		description: [
			...
			'done_at, result, broadcast }], count }. `body` is the brief in full, so a claimable task',
			'needs no second call to read. A task with `agent_id` set and `state` "todo" was assigned',
			'to that agent by the owner and only that agent may claim it.',
			'',
			"`broadcast` is true when the owner offered the task to this project's agents rather than",
			'to one of them — it is what raised your open_tasks count without anything being assigned',
			'to you. Every agent working the project was told, so claim it if you can take it and',
			'expect to lose the race sometimes.',
			'',
